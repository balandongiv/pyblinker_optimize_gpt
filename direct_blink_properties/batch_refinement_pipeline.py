...
1. **Per‑Technique Visual QA**
   ...
   * A ±10‑frame (≈⅓s @30fps) context window is added before/after each blink.
...
def process_all_fif_zip_pairs(fif_dir: str  | Path):
    ...
    # ]
    logger.info("Found %d FIF files and %d ZIP files", len(fif_files))
    ...
    for fif_path in fif_files:
        ...
        logger.info("Processing %s + %s", fif_path.name)
        try:
            raw,annotation_df = load_and_pick_channels(fif_path, channels)
            ...
            logger.exception("Error processing %s + %s: %s", fif_path.name, e)
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    FIF_DIR = Path(r"C:\Users\balan\OneDrive - ums.edu.my\CVAT_visual_annotation\temp_for_combine\dev")
    ...
    all_lb = process_all_fif_zip_pairs(FIF_DIR)
