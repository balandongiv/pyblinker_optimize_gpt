...
Usage:
------
Run directly as a script to execute the tests. Set the environment
variable ``PYBLINKER_SHOW_PLOTS=1`` to display plots:
    PYBLINKER_SHOW_PLOTS=1 python test_zero_crossing.py
...
# Use an environment variable so plotting can be toggled when running this file
# directly. Set ``PYBLINKER_SHOW_PLOTS=1`` to enable.
SHOW_PLOTS = os.environ.get("PYBLINKER_SHOW_PLOTS", "0") == "1"
...
def plot_zero_crossings(candidate_signal, max_blink, left_zero, right_zero,
                        ...
                        title: str = "Zero Crossing Visualization") -> None:
    """
    Plot zero-crossing markers on the candidate signal.

    The figure is only displayed if :data:`SHOW_PLOTS` is ``True``.
    """
